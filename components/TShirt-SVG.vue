...
onMounted(() => {
  let tshirt = document.getElementById("Ebene_1") as HTMLElement
  let imageOnShirt = document.getElementById("imageOnShirt") as HTMLElement
  let imageContainer = document.getElementById("imageContainer") as HTMLImageElement
  let shirtColor = document.querySelectorAll(".st0, .st1, .st2") as NodeList

  shirtColor.forEach((shirtSection) => {
    shirtSection.style.fill = store.data.color
  })
  ...
  watch(store.data, (oldV, newV) => {
    shirtColor.forEach((shirtSection) => {
      shirtSection.style.fill = newV.color
    })
    imageOnShirt.setAttribute("src", newV.img)
...
<template>
  ...
  <div class="showImage" id="imageContainer">
    <img src="" alt="" id="imageOnShirt">
  </div>
</template>
